    ...
    def generate_post_image_description(self):
        response = self.client.chat.completions.create(
          ...
          messages=[
            ...
            {"role": "user", "content": f"Составь подробный текстовый промпт для генерации изображения для соцсетей с темой {self.topic}. Промпт должен быть на английском языке, содержать детальное описание визуальных элементов, настроения и стиля изображения и быть длиной не более 1000 символов."}
          ]
        )
        return response.choices[0].message.content
